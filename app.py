...
def simulate(_config, reinvestment_strategy):
    ...
    for m in range(1, months + 1):
        ...
        if lucro_liquido_antes_distribuicao > 0:
            base_distribuicao = lucro_liquido_antes_distribuicao
        ...
        rows.append({
            ...
            "Módulos Alugados": modules_rented, "Módulos Próprios": modules_owned, "Receita": receita,
            "Manutenção": manut, "Aluguel": aluguel_mensal_corrente, "Parcela Terreno Inicial": parcela_terreno_inicial_mes,
            ...
            "Aporte": aporte_mes, "Fundo (Mês)": fundo_mes_total, "Retirada (Mês)": retirada_mes_efetiva,
            "Caixa (Final Mês)": caixa, "Investimento Total Acumulado": investimento_total,
            "Fundo Acumulado": fundo_ac, "Retiradas Acumuladas": retiradas_ac,
            "Módulos Comprados no Ano": novos_modulos_comprados, "Patrimônio Líquido": patrimonio_liquido
        })
...
if st.session_state.active_page == 'Configurações':
    ...
    cfg_g['years'] = c1.number_input("Horizonte (anos)", 1, 50, value=cfg_g['years'])
    cfg_g['general_correction_rate'] = c1.number_input("Correção Anual Geral (%)", 0.0, 100.0, value=cfg_g.get('general_correction_rate', 5.0), format="%.1f", key="global_corr_rate", help="Inflação anual que corrige receitas, custos, etc.")
    ...
    with st.expander("📅 Adicionar Eventos Financeiros (Aportes, Retiradas, Fundos)"):
        ...
        for i, aporte in enumerate(st.session_state.config['global']['aportes']):
            cols = st.columns([2, 3, 1]); aporte['mes'] = cols[0].number_input("Mês", 1, None, aporte['mes'], key=f"ap_m_{i}"); aporte['valor'] = cols[1].number_input("Valor (R$)", 0.0, None, aporte['valor'], format="%.2f", key=f"ap_v_{i}");
        ...
        for i, retirada in enumerate(st.session_state.config['global']['retiradas']):
            cols = st.columns([2, 3, 1]); retirada['mes'] = cols[0].number_input("Mês início", 1, None, retirada['mes'], key=f"ret_m_{i}"); retirada['percentual'] = cols[1].number_input("% do lucro", 0.0, 100.0, retirada['percentual'], format="%.1f", key=f"ret_p_{i}");
        ...
        for i, fundo in enumerate(st.session_state.config['global']['fundos']):
            cols = st.columns([2, 3, 1]); fundo['mes'] = cols[0].number_input("Mês início", 1, None, fundo['mes'], key=f"fun_m_{i}"); fundo['percentual'] = cols[1].number_input("% do lucro", 0.0, 100.0, fundo['percentual'], format="%.1f", key=f"fun_p_{i}");
...
if st.session_state.active_page == 'Dashboard':
    ...
    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True); st.markdown("### 🎯 Estratégias de Reinvestimento"); strat_cols = st.columns(3); config_copy = deepcopy(st.session_state.config)
        if strat_cols[0].button("**🏠 Comprar Novos**<br><small>Aquisição de terrenos próprios</small>", use_container_width=True):
            with st.spinner("Calculando..."): st.session_state.simulation_df = simulate(config_copy, 'buy'); st.session_state.comparison_df = pd.DataFrame()
        if strat_cols[1].button("**🏢 Alugar Novos**<br><small>Expansão com aluguel de terrenos</small>", use_container_width=True):
            with st.spinner("Calculando..."): st.session_state.simulation_df = simulate(config_copy, 'rent'); st.session_state.comparison_df = pd.DataFrame()
        if strat_cols[2].button("**🔄 Intercalar Novos**<br><small>Mix entre compra e aluguel</small>", use_container_width=True):
            with st.spinner("Calculando..."): st.session_state.simulation_df = simulate(config_copy, 'alternate'); st.session_state.comparison_df = pd.DataFrame()
    ...
    if not st.session_state.comparison_df.empty:
        st.markdown("<h2 class='gradient-header'>📈 Análise Comparativa</h2>", unsafe_allow_html=True); df_comp = st.session_state.comparison_df
        final_buy = df_comp[df_comp['Estratégia'] == 'Comprar'].iloc[-1]; final_rent = df_comp[df_comp['Estratégia'] == 'Alugar'].iloc[-1]; final_alt = df_comp[df_comp['Estratégia'] == 'Intercalar'].iloc[-1]
        ...
        with st.container(border=True):
            metric_options = ["Patrimônio Líquido", "Módulos Ativos", "Retiradas Acumuladas", "Fundo Acumulado", "Caixa (Final Mês)"]; selected_metric = st.selectbox("Selecione uma métrica para comparar:", options=metric_options)
            fig_comp = px.line(df_comp, x="Mês", y=selected_metric, color='Estratégia', color_discrete_map={'Comprar': PRIMARY_COLOR, 'Alugar': INFO_COLOR, 'Intercalar': WARNING_COLOR}); apply_plot_theme(fig_comp, f'Comparativo de {selected_metric}', h=450); st.plotly_chart(fig_comp, use_container_width=True)
        ...
        with col1, st.container(border=True):
            st.markdown("<h6>📈 Evolução do Patrimônio</h6>", unsafe_allow_html=True); fig_pat = go.Figure(); r, g, b = _hex_to_rgb(PRIMARY_COLOR)
            fig_pat.add_trace(go.Scatter(x=df["Mês"], y=df["Patrimônio Líquido"], name="Patrimônio", line=dict(color=PRIMARY_COLOR, width=4), fill='tozeroy', fillcolor=f'rgba({r}, {g}, {b}, 0.2)')); apply_plot_theme(fig_pat, "Crescimento do Patrimônio Líquido"); st.plotly_chart(fig_pat, use_container_width=True)
        with col2, st.container(border=True):
            st.markdown("<h6>🥧 Distribuição dos Recursos</h6>", unsafe_allow_html=True); dist_data = {"Valores": [final['Retiradas Acumuladas'], final['Fundo Acumulado'], final['Caixa (Final Mês)']], "Categorias": ["Retiradas", "Fundo Total", "Caixa Final"]}
            fig_pie = px.pie(dist_data, values="Valores", names="Categorias", color="Categorias", color_discrete_map={"Retiradas": DANGER_COLOR, "Fundo Total": INFO_COLOR, "Caixa Final": WARNING_COLOR}, hole=.5); apply_plot_theme(fig_pie, "Distribuição dos Recursos"); st.plotly_chart(fig_pie, use_container_width=True)
            
    else:
        st.markdown('<div class="card" style="text-align: center;">', unsafe_allow_html=True); st.info("🚀 Selecione uma estratégia de reinvestimento para iniciar a simulação."); st.markdown('</div>', unsafe_allow_html=True)
    ...
    if df_to_show.empty:
        ...
        with main_cols[1], st.container(border=True):
            ...
            if 'data_point' in locals() and not filtered.empty:
                ...
                fig_monthly = px.bar(chart_data, x="Categoria", y="Valor", text_auto='.2s', color="Categoria", color_discrete_map={"Receita": SUCCESS_COLOR, "Gastos": WARNING_COLOR, "Retirada": DANGER_COLOR, "Fundo": INFO_COLOR}); apply_plot_theme(fig_monthly, f"Fluxo Financeiro - Mês {int(data_point['Mês'])}"); st.plotly_chart(fig_monthly, use_container_width=True, config={'displayModeBar': False})
            else: st.info("Selecione um ponto no tempo.")
        
        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("Clique para ver a Tabela Completa da Simulação"):
            all_columns = df_analysis.columns.tolist(); state_key = f"col_vis_{slug(selected_strategy or 'default')}"
            if state_key not in st.session_state: default_cols = ['Mês', 'Ano', 'Módulos Ativos', 'Receita', 'Gastos', 'Caixa (Final Mês)', 'Patrimônio Líquido']; st.session_state[state_key] = {c: (c in default_cols) for c in all_columns}
            cols_to_show = st.multiselect("Selecione as colunas para exibir:", options=all_columns, default=[c for c, v in st.session_state[state_key].items() if v], key=f"multiselect_{state_key}")
